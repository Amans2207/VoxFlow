...
export default function AdminLogin() {
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    ...
    if (normalizedInputId === normalizedCfgId && password === configuredPassword) {
      // Set session storage for client-side quick check
      sessionStorage.setItem("vxf_admin_auth", "verified");
      
      // Set cookie for middleware protection
      document.cookie = "vxf_admin_auth=verified; path=/; max-age=86400; SameSite=Strict";
      
      router.push("/admin"); // Redirect to the new admin hub
    } else {
