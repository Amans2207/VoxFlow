...
const MagicPreview = dynamic(() => import("@/components/MagicPreview"), {
  ssr: false,
});
...
export default function Home() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      showToast("Neural Authentication Failed", "error");
    }
  };
    ...
    <div className={styles.container}>
        ...
        <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 0' }}>
          ...
          <nav style={{ display: 'flex', gap: '32px', alignItems: 'center' }}>
            ...
            <button 
              ...
              style={{ padding: '8px 24px', fontSize: '0.85rem', boxShadow: '0 0 30px var(--primary-glow)', background: 'var(--accent-blue)', color: 'white', border: 'none' }}
            >
              Get Started
            </button>
          </nav>
      ...
      <section className={styles.hero} style={{ paddingTop: '120px', paddingBottom: '60px' }}>
        ...
        <motion.h1 
          ...
          style={{ fontSize: 'clamp(3rem, 8vw, 6rem)', marginBottom: '32px', lineHeight: '0.85', fontWeight: 900, letterSpacing: '-0.05em' }}
        >
          AI Production <br/>
        ...
        <motion.p 
          animate={{ opacity: 1, y: 0 }}
          style={{ fontSize: 'clamp(1rem, 2vw, 1.5rem)', maxWidth: '800px', margin: '0 auto 48px', color: 'var(--text-secondary)', lineHeight: '1.4', fontWeight: 500 }}
        >
      ...
      {/* Pricing Section */}
      <section id="pricing" className={styles.section} style={{ marginTop: '120px', paddingBottom: '120px' }}>
        <div className={styles.sectionHeader} style={{ textAlign: 'center', marginBottom: '80px' }}>
          <div className={styles.sectionLabel}>Plans & Scale</div>
          <h2 style={{ fontSize: '4.5rem', fontWeight: 900 }}>Ready to go Global?</h2>
        </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 px-4">
          <PricingCard 
            tier="Free" 
            price="₹0" 
            features={["10 Neural Credits", "Standard Voice Engine", "Watermarked Export", "720p Quality"]} 
            delay={0.1}
          />
          <PricingCard 
            tier="Creator" 
            price="₹999" 
            isPopular
            features={["100 Neural Credits", "Premium Voice Engine", "No Watermark", "1080p Export", "Priority Queue"]} 
            delay={0.2}
          />
          <PricingCard 
            tier="Pro" 
            price="₹2,499" 
            features={["500 Neural Credits", "Ultra-HD 4K Export", "Bulk Orchestrator", "Dedicated Account Manager"]} 
            delay={0.3}
          />
        </div>
      </section>
      ...
      <footer className="voxflow-footer">
          <div className="footer-content">
              ...
              <div className="footer-section contact">
                  ...
                  <div style={{ marginTop: '20px', display: 'flex', gap: '16px', fontSize: '0.75rem', opacity: 0.6 }}>
...
function PricingCard({ tier, price, features, isPopular, delay }: any) {
  return (
    <motion.div 
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      transition={{ delay }}
      className={`p-10 rounded-3xl border ${isPopular ? 'border-blue-500 bg-blue-500/5' : 'border-white/10 bg-white/5'} backdrop-blur-xl relative overflow-hidden`}
    >
      {isPopular && (
      ...
      <div className="text-5xl font-black mb-8">{price}<span className="text-sm text-gray-500 font-medium">/mo</span></div>
      ...
      <Link href="/signup">
        <button className={`w-full py-4 rounded-xl font-bold transition-all ${isPopular ? 'bg-blue-500 text-white hover:bg-blue-400 shadow-[0_0_30px_rgba(0,102,255,0.3)]' : 'bg-white/10 text-white hover:bg-white/20'}`}>
          Scale Now
        </button>
      </Link>
    </motion.div>
