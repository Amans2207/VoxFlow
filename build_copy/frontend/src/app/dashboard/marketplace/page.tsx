...
export default function Marketplace() {
  ...
  const startSynthesis = async () => {
    ...
    // Credit Check
    const cost = selectedTemplate.price || 5.0;
    if (balance < cost) {
       if (window.confirm("Insufficient Neural Balance for this template. Refill now?")) {
    ...
    try {
      const data = await safeFetch('/api/marketplace/process', {
        method: 'POST',
       ...
       <AnimatePresence>
         {selectedTemplate && (
           ...
           >
             ...
             >
               <button 
                 onClick={() => { setSelectedTemplate(null); setUploadedUrls([]); setResultUrl(null); }}
                  ...
                  {!resultUrl ? (
                    <div className="flex flex-col gap-10">
                       ...
                       <button 
                         ...
                         className={`h-20 w-full rounded-3xl font-black text-[12px] uppercase tracking-[6px] transition-all border ${uploadedUrls.length === 0 || isProcessing ? 'bg-white/5 border-white/5 text-zinc-600 cursor-not-allowed' : 'bg-[#10b981] border-[#10b981] text-black cursor-pointer hover:shadow-[0_0_30px_rgba(16,185,129,0.3)]'}`}
                       >
